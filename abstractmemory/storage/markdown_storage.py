...
class MarkdownStorage(IStorage):
    ...
    def _create_directories(self):
        ...
        directories = [
            "verbatim",      # Deterministic verbatim records (100% factual)
            "notes",         # LLM-generated experiential notes (subjective)
            "experiential",  # Legacy folder (for backwards compatibility)
            "links",         # Bidirectional links between verbatim and notes
            "core",          # Core memory snapshots
            "semantic"       # Semantic memory snapshots
        ]
    ...
    def _create_interaction_markdown(self, interaction_id: str, user_id: str,
        ...
        """
        Create deterministic verbatim markdown for interaction.

        CRITICAL: This is a VERBATIM record written deterministically by the system.
        Format: user, time, location, query, response - NO interpretation.
        The LLM subjective experience goes in experiential notes (notes/).
        """
        # Extract location from metadata (required field)
        location = metadata.get('location', 'unknown') if metadata else 'unknown'

        # Deterministic header: user, time, location
...
"""
        ...
        # Core verbatim content (deterministic)
...
"""
        ...
        # Optional metadata (if present, show at bottom)
        if metadata:
...
"""
    ...
    def save_experiential_note(self, timestamp: datetime, reflection: str,
        ...
        """
        Save AI experiential note (LLM-generated subjective interpretation).

        CRITICAL: The 'reflection' parameter should contain LLM-generated content
        (>90% of the note). Only minimal template (participants, time, location) is added.

        This is the LLM's SUBJECTIVE EXPERIENCE, not a deterministic log.
        """
        # Generate note ID
        note_id = f"note_{uuid.uuid4().hex[:8]}"

        # Create file path - use "notes/" folder as per spec
        date_path = self._get_date_path(timestamp)
        time_prefix = self._get_time_prefix(timestamp)
        topic = metadata.get('topic', 'reflection') if metadata else 'reflection'
        filename = f"{time_prefix}_{topic}_{note_id}.md"

        # Use "notes/" instead of "experiential/" to match spec
        file_path = self.base_path / "notes" / date_path / filename
        file_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        # Update index (keep key as "experiential_notes" for backwards compatibility)
    ...
    def _create_experiential_markdown(self, note_id: str, timestamp: datetime,
        ...
        """
        Create markdown for LLM-generated experiential note.

        CRITICAL: >90% of content should be the LLM-generated 'reflection'.
        Template provides minimal structure: participants, time, location only.
        """
        # Extract minimal context from metadata
        user_id = metadata.get('user_id', 'unknown') if metadata else 'unknown'
        location = metadata.get('location', 'unknown') if metadata else 'unknown'

        # MINIMAL template header (participants, time, location)
...
"""
        ...
        # THE MAIN CONTENT - LLM-generated subjective experience (>90%)
...
"""
        ...
        # Footer (minimal)
        content += f"""*This is a subjective experiential note generated by the AI*
