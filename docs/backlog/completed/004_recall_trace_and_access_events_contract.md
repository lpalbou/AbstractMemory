# Completed: Recall trace and access events contract
...
Shipped by the engine wave: selection traces + attention events (journal families trace/event), commit_selection as the one strengthening path, recall_history/absence_diagnosis reads, and the frozen export_replay stream (a2a 0005) as the audit surface. The item's no-prompt-policy rule holds (traces are audit-only).
