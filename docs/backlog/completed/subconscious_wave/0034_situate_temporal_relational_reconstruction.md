# Completed: situate() — temporal + relational context reconstruction
...
Shipped: situate.py (`situate()` + `situate_prompt_block()`, system facade method) — pure-read temporal/relational reconstruction, always-historical labels, anchored reads; consumed by the two-anchor summon work (R3 read-only reincarnation) and runtime grounding.
