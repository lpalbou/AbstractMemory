# Completed: open_commitments() + trigger annotation (prospective memory)
...
Shipped: `open_commitments()` (diary.py, fulfills= convention, believed-rows fold), commitments in drive_pressure/alive_drives (the day surfaces), and runtime taught the election + fulfills discharge. Receipts: extension-wave ship; runtime's 277 pathway wave named commitments joining the offer vocabulary.
