# Completed: familiarity() — pre-answer metamemory read
...
Shipped: `familiarity()` (probe.py, root-exported) — density-not-content metamemory over exact/vector/keyword/participants; FAMILIARITY_VECTOR_MIN recalibrated to 0.55 on live-store measurement (qwen3-0.6b space); keyword discovery A/B'd and deliberately left OFF for this read. Consumed live by the runtime driver's anti-fabrication reflex. Receipts: extension-wave ship + the 2026-07-19 recalibration wave; suite-pinned.
