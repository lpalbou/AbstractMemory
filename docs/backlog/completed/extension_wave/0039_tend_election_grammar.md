# Completed: the ```tend election grammar (attention + repair agency)
...
Shipped: `parse_tend_block()` + the tend election lane (redigestion, close/supersede acts, #tag resolution runtime-side per K-blocker fix seq 128/130). Receipts: extension-wave ship + runtime's tend wiring.
