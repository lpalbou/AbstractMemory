# Completed: SQLite database compatibility and store capabilities
...
Shipped in substance across the engine + entity waves: SQLiteTripleStore with native vectors + FTS5, WAL/checkpoint close, BEGIN IMMEDIATE chained appends, incoming-edge index, mtime_ns/inode identity lessons, embedding pin (M1) + reembed_store/reembed_home. The remaining capabilities-surface question folds into proposed/memory_system_v1/0015 (kept open).
