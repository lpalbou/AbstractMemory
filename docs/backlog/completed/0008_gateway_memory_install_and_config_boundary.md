# Completed: Gateway Memory Install And Configuration Boundary
...
Closed against current reality: the boundary this item asked for exists on both sides. Gateway owns store selection through its own resolver (`abstractgateway/memory_store.py` — backend map incl. sqlite/lancedb, env-driven config) and embedding provider config through `/api/gateway/embeddings`; entity homes construct SQLiteTripleStore directly with the M1 embedding pin. Memory stayed dependency-light (store classes + `AbstractGatewayTextEmbedder` over HTTP; no gateway/provider knowledge in-package). The hardcoded-LanceDB problem the item named is gone.
