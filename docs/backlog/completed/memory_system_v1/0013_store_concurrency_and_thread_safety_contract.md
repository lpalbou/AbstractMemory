# Completed: Store concurrency and thread-safety contract
...
Shipped: SQLite BEGIN IMMEDIATE chained appends, per-instance append locks, WAL close/checkpoint, the one-writer-per-directory lease (runtime storage/lease.py) as the cross-process arbiter, and the documented single-writer contract (M2, consensus item).
