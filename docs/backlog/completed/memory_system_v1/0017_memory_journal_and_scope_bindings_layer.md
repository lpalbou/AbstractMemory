# Completed: Memory journal (events/traces/snapshots) + append-only scope bindings
...
Shipped: journal.py protocol + InMemory/SQLite backends — events, bindings (fold semantics), closures, traces, snapshots, valence; one monotonic seq axis; as_of replay anchoring.
