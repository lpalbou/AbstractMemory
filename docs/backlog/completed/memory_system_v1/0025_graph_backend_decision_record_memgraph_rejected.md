# Completed: Graph backend decision record — Memgraph rejected, promotion criteria for embedded alternatives
...
Decision record executed: Memgraph rejected (daemon-only, BSL, persistence footguns); SQLite shipped as the durable home store with native vectors; the decision is engraved in module docs.
