# Completed: Source-linked summaries and derived assertion lineage
...
Shipped by the engine wave: the summary guard ('a summary must name its sources' — summarizes edges required at formation), redigestion lineage (refines + redigested_from + supersede closures), world-model card revision chains (known_source_ids, derived_from), and reified relation rows carrying provenance.
