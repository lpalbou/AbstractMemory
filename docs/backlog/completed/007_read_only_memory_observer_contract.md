# Completed: Read-only memory observer contract
...
Shipped as the frozen replay stream v1 (a2a 0005): export_replay over six journal families, display blocks (graph_id join, diary redaction, signals), SSE serving via the gateway, and the observer's entity view consuming it. Pure-read by construction.
